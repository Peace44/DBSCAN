...
void GroundSegmentation::init(GroundSegmentationParams& params) {
  params_ = params;

  segments_ = std::vector<Segment>(params.n_segments, Segment(params.n_bins,
                                         params.min_slope,
                                         params.max_slope,
...
void GroundSegmentation::segment(const PointCloud& cloud, std::vector<int>* segmentation) {
  ...
  if (params_.visualize) {
    boost::shared_ptr<PointCloud> obstacle_cloud = boost::make_shared<PointCloud>();
    obstacle_cloud->reserve(segmentation->size() - n_ground);
    // Get cloud of ground points.
    boost::shared_ptr<PointCloud> ground_cloud = boost::make_shared<PointCloud>();
    ...
    boost::shared_ptr<PointCloud> min_cloud = boost::make_shared<PointCloud>();
...
void GroundSegmentation::insertionThread(const PointCloud& cloud,
  ...
  for (unsigned int i = start_index; i < end_index; ++i) {
    ...
    if (range_square < params_.r_max_square && range_square > params_.r_min_square) {
      ...
      const unsigned int segment_index_clamped = (int)segment_index == params_.n_segments ? 0 : segment_index;
